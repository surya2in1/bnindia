...
--------------------------------------------------------------------------------------------
 -- testing
 SELECT m.month,DATE_FORMAT(p.due_date, '%M') AS p_month, IFNULL(SUM(total_amount), 0 ) AS total_amount FROM months m LEFT JOIN payments p on m.month=DATE_FORMAT(due_date, '%M') WHERE ((YEAR(p.due_date) = YEAR(CURDATE()) OR YEAR(p.due_date) is null) AND (p.created_by = 1 OR p.created_by is null)) GROUP BY m.month ORDER by m.id ASC
